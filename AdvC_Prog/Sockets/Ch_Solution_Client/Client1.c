...
else
{
    ...
    strcpy(msg_to_server, "123");

    //Send the message to server
socketSend(socket_desc, msg_to_server, strlen(msg_to_server));

// Receive Data From Server
read_size = socketReceive(socket_desc,msg_from_server,200);

int num = atoi(msg_from_server);
