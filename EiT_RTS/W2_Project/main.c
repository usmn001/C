...
TickType_t xInitial_Time, xFinal_Time;
...
{
	while (1) {
		xInitial_Time=xTaskGetTickCountFromISR();
		printf("Sending data...\n");
		fflush(stdout);
		vTaskDelay(100);
		printf("Data sent!\n");
		fflush(stdout);
		vTaskDelay(100);
		xFinal_Time = xTaskGetTickCountFromISR()-xInitial_Time;
		prioritysettask(xFinal_Time);
	}
}

void prioritysettask(TickType_t xFinal_Time)
{
	if (xFinal_Time>1000)
