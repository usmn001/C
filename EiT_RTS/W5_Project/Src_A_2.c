...
{
	struct AMessage am;
	struct AMessage *pw_s=NULL;                                        // Pointer For Writing Strucutre Used For Sending Data To Queue

	Q1_handle=xQueueCreate( ROW, sizeof( struct AMessage*  ) );
		if(Q1_handle!=0)		{
	...
	while (1) {
		...
		*/
		...
		am.QData=c;
		pw_s=&am;
		if(  xQueueIsQueueFullFromISR(Q1_handle)==pdFALSE  ){
		if(xQueueSend(Q1_handle,(void*)&pw_s,( TickType_t ) 0 )!=pdPASS) {
		printf("Queue Post Failed\n");
		}
		}
...
{
struct AMessage *pr_s=NULL;                 // Pointer For Copying Received Data From Queue
while(1)
{
if(uxQueueMessagesWaiting(Q1_handle)>0){
			if(xQueueReceive(Q1_handle,&(pr_s),( TickType_t ) 0 )!=pdPASS)  {
						printf("Queue Received Failed\n");
					}
			else
				{
				double (**a)=pr_s->QData;
...
int main( void )
{
	...
	xTaskCreate(reader_task,"Reader",500, NULL, 3, &reader_handle);
